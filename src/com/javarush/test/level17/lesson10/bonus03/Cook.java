        ...
        while (continueWorking || needToCookOrders()) {
            try {
                synchronized (this) {
                    ...
                    if (!needToWait) {
//                        System.out.println("Всего заказов: " + Manager.getInstance().getOrderQueue().size());
                        cooking();
                    }
                }
...
//        System.out.println(String.format("Заказ для стола №%d на выдаче, блюдов к выдаче: %d", dishes.getTableNumber(), Manager.getInstance().getDishesQueue().size()));
    }
}
